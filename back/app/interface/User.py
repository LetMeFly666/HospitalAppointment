...
def apply2be1caregiver(request):
    ...
    runner = models.Runner(id=userid, name=name, phone=phone, status='待联系')
    runner.save()
...
def getFriends(request):
    ...
    userid = models.User.objects.get(warrant=warrant).userid
    friends = models.Friend.objects.filter(friend=userid, using='y').values()
...
def delete1friend(request):
    ...
    userid = models.User.objects.get(warrant=warrant).userid
    models.Friend.objects.filter(id=friendId, friend=userid).update(using='n')
...
def create1order(request):
    ...
    name += type_
    amount = int(type_.split('/')[0].split('￥')[1]) * 100  # 取消注释并删除下行
    # amount = 1  # 这一行是调试用
